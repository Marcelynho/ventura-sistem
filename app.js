...
const valorRestante = Math.max(valorTotal - valorEntrada, 0);
    const dadosOS = {
        ...
        tratamento: document.getElementById("tratamentoOS")?.value || "",
...
async function imprimirOSModelo(tipo){
  ...
  <div class="box"><h3>Produto</h3><p><b>Lente:</b> ${g("lenteOS")}</p><p><b>Armação:</b> ${g("armacaoOS")}</p><p><b>Tratamento:</b> ${g("tratamentoOS")}</p></div>
...
async function buscarOS() {
  ...
  (os.cpf && String(os.cpf).includes(busca))
    ...
    if (document.getElementById("tratamentoOS")) document.getElementById("tratamentoOS").value = os.tratamento || "";
